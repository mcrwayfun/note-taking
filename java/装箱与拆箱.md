...
### 2.3 装箱和拆箱是如何实现的
那么装箱和拆箱是如何实现的呢，使用**javap -c**指令对2.2的代码进行反编译，可以看到：
...
#### 使用new Integer(xxx)不会触发自动装箱，而Integer a = xxx则会
#### 对于表达式会出发自动装箱和拆箱
```java
	Integer a = 1;
	Integer b = 2;
	...
	false
```
- 上述例子`c==(a+b)`为true，此时==是比较两个数值是否相等，而不是比较是否指向同一个对象。因为a+b会先触发自动拆箱，得到1和2，此时与c比较，数值上相等
- `c.equals(a+b)`为true，a和b会自动触发自动拆箱（intValue），接着a+b得到3后会触发自动装箱，所以equals比较数值，`c.equals(a+b)`为true
- `d==(a+b)`为true，因为a+b会先触发自动拆箱，得到1和2，然后自动转为long类型，所以为true
- `d.equals(a+b)`为false，equals不会自动转换类型，所以3L和3比较为false
